...
const PostCreation = ({ user, isDark }) => {
  ...
  return (
    <div className={`rounded-lg shadow mb-4 p-4 dark:bg-black  bg-white`}>
		
      <div className='flex space-x-3'>
        <img src={user.profilePicture || "/avatar.png"} alt={user.name} className='size-12 rounded-full ' />
        <textarea
          placeholder="What's on your mind?"
          className={`w-full p-3 rounded-lg dark:border-white border-black border-[1px] dark:bg-black dark:text-white text-black bg-white focus:outline-none resize-none transition-colors duration-200 min-h-[100px]`}
          value={content}
      ...
      <div className='flex justify-between items-center mt-4'>
        <div className='flex space-x-4 '>
          <label className='flex items-center text-info hover:text-info-dark transition-colors duration-200 cursor-pointer dark:text-white text-black'>
        ...
        <button
          className={`rounded-lg px-4 py-2 transition-colors duration-200 ${isDark ? 'bg-primary' : 'bg-primary text-white hover:bg-primary-dark'}`}
