...
const HomePage = ({ isDark }) => {
	...
	const { data: posts2 } = useQuery({
		...
		queryFn: async () => {
			const res = await axiosInstance.get(`/admin/getUserPosts/${authUser?._id}`);
			return res.data;
		},
	});

	return (
		<div className={`grid ${isDark ? 'dark:bg-gray-900' : 'bg-white'} grid-cols-1 lg:grid-cols-4 gap-6`}>
			...
			<div className='col-span-1 lg:col-span-2 order-first lg:order-none'>
				...
				{YourPost ? (
					posts2?.length > 0 ? (
						...
						<div className={`bg-white ${isDark ? 'dark:bg-gray-800' : 'bg-white'} rounded-lg shadow p-8 text-center`}>
							<div className="mb-6">
							...
							<p className={`text-gray-600 mb-6 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
			...
			{recommendedUsers?.length > 0 && (
				<div className="col-span-1 w-[22rem] lg:col-span-1 hidden lg:block">
					<div className={`bg-secondary rounded-lg shadow p-4 ${isDark ? 'dark:bg-gray-800' : 'bg-white'}`}>
						<h2 className={`font-semibold mb-4 ${isDark ? 'text-white' : 'text-black'}`}>People you may know</h2>
						...
						<div className="flex mb-4">
							<input
								...
								className={`border w-full rounded-l-lg p-2 flex-grow ${isDark ? 'bg-gray-700 text-white' : 'bg-white text-black'}`}
							/>
							<button
						...
						{filteredUsers?.map((user) => (
							<RecommendedUser key={user._id} user={user} isDark={isDark} />
						))}
					</div>
				</div>
			)}
